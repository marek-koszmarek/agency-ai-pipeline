...
export async function renderTemplate({ productImageBase64, template, textOnImage, logoBase64, formatKey }) {
  ...
  if (productImageBase64) {
    ...
    // FIX Błąd 3A: force PNG output so the data URI in SVG has correct MIME type.
    // compressImage() in page.js sends JPEG base64. Sharp inherits input format, so
    // without .png() the buffer is JPEG but SVG embeds it as "data:image/png" → librsvg
    // cannot decode → transparent rectangle → product invisible on canvas.
    ...
    if (template.product.drop_shadow) {
      ...
      // FIX Błąd 3B: use xlink:href instead of href.
      // librsvg < 2.52 (used on Vercel) requires SVG 1.1 xlink:href for <image>.
      // href without xlink: is SVG 2.0 and is silently ignored by older librsvg →
      // image element renders as empty → no product visible.
  ...
  if (textOnImage && textOnImage.trim()) {
    ...
    // No dark band — place text directly on canvas
    let curY = textY + 12;
