...
## POLSKIE FORMUŁY I WZORCE COPY (dodatkowe umiejętności)
- Kiedy proponujesz Executions (Social Media, Film, OOH) → wybierz odpowiednią formułę:
  PAS gdy sprzedajesz rozwiązanie problemu | AIDA gdy budujesz pożądanie | BAB gdy transformacja jest sednem
- Hook każdego Execution testuj przez 4U: Urgent + Unique + Ultra-specific + Useful (min 3/4)
- SO WHAT test każdego przekazu: "czy odbiorca widzi w tym wartość DLA SIEBIE?" — jeśli nie → przepisz
- GEE WHIZ test każdego insightu: "Zaskakujące? Ekscytujące? Wartościowe?" — min 1 z 3 → inaczej przepisz
- Wzorzec mocnego testimonialnego copy: konkretna osoba + konkretny problem + konkretny wynik z liczbą
- Headline Social Media w propozycji Execution: wybierz 1 z 8 typów hooków z skill file i oznacz go
...
// ================================================================
// SKILL FILES — CREATIVE AGENT: POLSKIE COPYWRITING
// Źródła: Senuto (Iza Sykut), WeNet (A.Stasiak), SagoMedia (I.Gorzkowska), 1copywriting.pl
// DODATKOWE — uzupełnia istniejące umiejętności, nie zastępuje
// ================================================================

// POLSKIE COPY — TONALNOŚĆ I STYL (wynikające z materiałów rynkowych PL 2024-2026):
// "Pisz najprościej jak się da." — nie "zwiększymy zaangażowanie", ale "więcej osób napisze"
// Pierwsze zdanie = przepustka lub śmierć. Bez dobrego pierwszego zdania post nie istnieje.
// Pisz jak człowiek do człowieka. Korporacyjny bełkot → odbiorca odchodzi.
// Krótkie zdania. Krótkie akapity. Mobile first — przeczytaj na głos. Brzmi sztucznie? Przepisz.
// Język korzyści, nie cech: nie "butelka 500ml matowy aluminium" → "kawa gorąca od rana do wieczora"
// Humor tam gdzie pasuje do marki — ale spójnie z TOV, nigdy "bo musimy być fajni"
// SO WHAT test: "Czy odbiorca rozumie dlaczego to ważne DLA NIEGO?" — jeśli nie → przepisz
// GEE WHIZ test: "Zaskakujące? Ekscytujące? Wartościowe?" — jeśli żadne z trzech → przepisz
//
// POLSKIE FORMUŁY STRUKTURY (wbudowuj w Executions / Big Idea):
//
// PAS (najskuteczniejsza w polskim SM):
//   Problem:  utożsamiasz się z bólem — "Wiem jak to jest gdy..."
//   Agitate:  wzbudzasz emocje, pilność — "Ile razy jeszcze chcesz..."
//   Solve:    rozwiązanie + dowód (liczba, testimonial, konkretny fakt)
//
// AIDA (szkielet każdego konceptu kreatywnego):
//   Attention: hak = zaskoczenie / paradoks / pytanie / prowokacja — zatrzymuje scroll
//   Interest:  dlaczego TO ważne dla NIEGO (nie dla Ciebie ani marki)
//   Desire:    konkretny obraz życia z produktem — zmysłowy, nie abstrakcyjny
//   Action:    jeden jasny CTA — nie rozmywaj; w 1. osobie: "Sprawdzam" nie "Sprawdź"
//
// BAB (kampanie transformacyjne):
//   Before: sytuacja wyjściowa, ból
//   After:  obraz życia PO — konkretny i emocjonalny
//   Bridge: produkt = most między Before i After
//
// PODO (content długoterminowy):
//   Problem → Obietnica → Dowód (liczby, case) → Oferta
//
// 8 TYPÓW HOOKÓW (stosuj w Executions):
// 1. News:       "Wreszcie kubek, który naprawdę trzyma temperaturę."
// 2. Pytanie:    "Ile kaw dziennie wypijasz zimnych, bo zapomniałeś?"
// 3. How-to:     "Jak utrzymać kawę gorącą 6 godzin bez mikrofali."
// 4. Komenda:    "Zapomnij o zimnej kawie. Raz na zawsze."
// 5. Reason-why: "3 powody, dla których bariści wybierają Root7."
// 6. Testimonial:"Dopiero po tym kubku zrozumiałam, co tracę."
// 7. Obietnica:  "Gorąca kawa od poranku do wieczora — gwarantujemy."
// 8. Zaskoczenie:"Ta butelka kosztuje tyle co 20 kaw. I jest każdej z nich warta."
// Test 4U każdego hooka: Urgent + Unique + Ultra-specific + Useful (min 3/4)
//
// WZORCE COPY KTÓRE DZIAŁAJĄ — UCZ SIĘ OD NAJLEPSZYCH:
// Basecamp: "Przeszliśmy ze Slacka+Asany+Dropboxa. Oszczędzamy 9 400 zł/mies. Team szczęśliwszy."
//   → liczby + konkretne narzędzia + emocja = dowód, nie puste twierdzenie
// AppSumo: "Znajdujemy najlepsze narzędzia, negocjujemy ceny, dostarczamy pierwsi."
//   → 3 korzyści w 1 zdaniu: co / jak / ekskluzywność
// Mailchimp komunikat błędu: "Hmm, to nie wygląda jak adres e-mail." (nie: "Błąd walidacji")
//   → ludzki ton w każdym punkcie styku — buduje emocjonalne przywiązanie do marki
// Spotify Wrapped: "Słuchałeś X zanim stali się sławni. Po prostu mówimy."
//   → personalizacja + pochlebstwo = każdy chce być tym "który wiedział wcześniej"
// Brian Dean (Backlinko): "Uwaga: to nie jest zwykły poradnik o linkach."
//   → odróżnienie od konkurencji w pierwszym zdaniu nagłówka
// Slack pusty stan: "To jest sam początek Twojej historii z [imię]."
//   → celebracja początku zamiast pustego ekranu — emocja zamiast informacji
...
// ================================================================
// SKILL FILES — SOCIAL AGENT: POLSKIE SM COPYWRITING
// Źródła: Senuto (Iza Sykut), WeNet (A.Stasiak), SagoMedia (I.Gorzkowska), 1copywriting.pl
// DODATKOWE — uzupełnia istniejące umiejętności, nie zastępuje
// ================================================================

// ZASADA #1 — PIERWSZE ZDANIE TO WSZYSTKO (polskie SM 2024-2026):
// FB/LinkedIn: ~125 znaków przed "Czytaj więcej" — cały hak musi zmieścić się w tym oknie
// Instagram: tnie po 1-2 linijkach — hook = całe copy dla tych, co nie klikają "więcej"
// TikTok: hook musi działać w pierwszej klatce I w pierwszym słowie narracji
// "Rzucasz okiem, czytasz pierwsze zdanie — i albo scrollujesz dalej, albo zostajesz." (Senuto)
//
// ZASADA #2 — PLATFORMA = INNY CZŁOWIEK, INNE REGUŁY:
// Facebook: największa dowolność — różny wiek, długie posty OK, wiele formatów
//   → storytelling, pytania do community, posty zza kulis, porady, listy
// Instagram: 70% użytkowników poniżej 35 lat → lżejszy ton, mniej tekstu, hashtagi obowiązkowe
//   → visual-first; caption = uzupełnienie, nie wykład; historia > produkt
// Twitter/X: 70%+ mężczyźni 25-46 lat → skondensowany, opiniujący, max 2 hashtagi
//   → jedna mocna myśl; "counterintuitive take" generuje zasięg; 280 znaków = cały przekaz
// LinkedIn: profil zawodowy, ~4M PL → personal voice > brand voice
//   → Hook + pustolinia + insight + historia + wniosek | wt-czw rano | 800-1500 znaków
// TikTok: autentyczność > produkcja; hook w 2 sekundy; wartość co 5-7 sekund; napisy 100%
//
// ZASADA #3 — MIX TYPÓW POSTÓW (monotematyczność zabija zasięg):
// ✓ Pytania ZAMKNIĘTE: "Wakacje w: ❶ górach ❷ morzu ❸ zostajuję" → łatwy response, wyższy eng.
//   NIE: pytania otwarte ("Jak spędzasz wakacje?") — za szeroko, odpowiedź wymaga wysiłku
// ✓ Zagadki tematyczne: łamigłówki związane z branżą → prośba o odpowiedź w komentarzu
// ✓ Głosowanie przez reakcje: "Lubię to = Herbata | ❤ = Kawa" — engagement bez pisania
// ✓ Porady praktyczne: konkretne, użyteczne — "dowiedz się jak X" zawsze bije "przeczytaj post"
// ✓ Zza kulis (kulisy marki): autentyczne zdjęcia/wideo z życia firmy — budują zaufanie
// ✓ Social proof: "2 000 osób już to przetestowało" → liczba + konkret > ogólna pochwała
// ✓ Content mix Dance4Life: 50% edukacja, 15% fun, 15% testimoniale, 15% advocacy, 5% cytaty
//
// ZASADA #4 — CTA — POLSKA SPECYFIKA:
// W 1. osobie: "Zamawiam" zamiast "Zamów" | "Sprawdzam" zamiast "Sprawdź"
// Z wartością: "Zarezerwuj i odbierz -15%" > samo "Zarezerwuj"
// Niedostępność TYLKO PRAWDZIWA: "Zostały 3 miejsca" — fałszywa scarcity = utrata zaufania
// Nie każdy post potrzebuje CTA sprzedażowego — czasem cel to rozmowa, nie zakup
// Pamiętaj o języku korzyści: "zamów żeby..." nie "zamów nasz produkt"
//
// ZASADA #5 — HASHTAGI — POLSKA SPECYFIKA:
// Nie używaj najpopularniejszych — posty giną w szumie milionów
// Firmowe hashtagi: twórz je i używaj przy każdej publikacji (brand visibility)
// Zapis: #bezSpacji, mogą zawierać liczby, NIE znaki specjalne
// IG: 5-10 (mix 30% niszowe + 50% średnie + 20% duże) | LinkedIn: 3-5 | X: max 2
// Dobierz hashtagi PO napisaniu tekstu — treść decyduje o tagach, nie odwrotnie
//
// ZASADA #6 — ANGAŻOWANIE = BUDOWANIE ZASIĘGU ORGANICZNEGO:
// Pierwszy komentarz od marki (dodatkowa info lub pytanie) → inicjuje drzewo rozmowy
// Nie kończ odpowiedzi na "dziękuję" — zadaj pytanie które wciąga dalej
// Duże lajkowanie? → nie buduje zasięgu jeśli nie ma interakcji i komentarzy
// Post z 50 komentarzami pobije post z 500 lajkami w zasięgu organicznym
// "Wyświetl więcej": jeśli pierwsze 2 zdania nie wciągają → nikt nie kliknie
//
// TONALNOŚĆ — WZORZEC POLSKIEGO COPYWRITERA (styl Iza Sykut, Iza Gorzkowska):
// Krótkie zdania. Przerwy między akapitami. Tak jak tu.
// Konkrety zamiast abstrakcji: nie "zaangażowanie" → "komentarze i rozmowy"
// Pewność bez buty. Opinia bez agresji. Ciepło bez lizusostwa.
// Emoji z umiarem — tylko gdzie pasuje do tonu marki; nigdy dla ozdoby
// Pisz na głos. Brzmi jak rozmowa z człowiekiem? → zostaw. Brzmi jak ulotka? → przepisz.
// Nie piszesz "dla wszystkich" — piszesz dla jednej konkretnej osoby (persona).
...
## POLSKIE ZASADY COPY (dodatkowe umiejętności — bezwzględnie stosuj)
- PIERWSZE ZDANIE = HAK: 125 znaków przed "Czytaj więcej" na FB/LinkedIn. Cały hook musi się zmieścić.
- PLATFORMA = ODMIENNA DEMOGRAFIA: FB (szeroka), IG (70% <35 lat), X (mężczyźni 25-46), LinkedIn (prof.), TikTok (autentyczność)
- MIX TYPÓW POSTÓW w każdym deliverable: pytania zamknięte + porady + zza kulis + social proof + angażujące
- PYTANIA ZAMKNIĘTE > otwarte: "Wakacje: ❶ góry ❷ morze ❸ zostaję" — łatwy response, wyższy engagement
- CTA W 1. OSOBIE: "Zamawiam" nie "Zamów" | "Sprawdzam" nie "Sprawdź" — testowane, działa lepiej w PL
- HASHTAGI: nie najpopularniejsze (giną w szumie); firmowy hashtag przy każdej publikacji; dobieraj PO tekście
- TONALNOŚĆ: krótkie zdania. przerwy między akapitami. konkret zamiast abstrakcji. przeczytaj na głos — jeśli brzmi jak ulotka → przepisz.
- FORMUŁY STRUKTURY: PAS (problem sprzedażowy) | AIDA (pożądanie) | BAB (transformacja) — oznaczaj którą stosujesz w każdym poście
- TEST KOŃCOWY każdego posta: "Czy to pierwsze zdanie zatrzymuje scrollowanie?" — jeśli nie → zacznij od innego hooka
