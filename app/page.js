...
function DropZone({ files, onFiles, label, hint, accept = ACCEPTED, single = false }) {
  ...
  const add = useCallback(async (fl) => {
    ...
    onFiles((p) => {
      const n = new Set((Array.isArray(p) ? p : []).map(f => f.name));
      return [...(Array.isArray(p) ? p : []), ...prep.filter(f => !n.has(f.name))];
    });
  }, [onFiles, single]);
  return (
    <div>
        ...
        <div className="file-chips">
          {(Array.isArray(files) ? files : []).map((f) => (
            <span key={f.name} className="file-chip">
              {f.name}
              <button onClick={() => onFiles(Array.isArray(files) ? files.filter(x => x.name !== f.name) : [])}>×</button>
            </span>
          ))}
        </div>
      )}
...
function ResultTabs({ mode, results, extraTabs = [] }) {
  const baseTabs = (STEPS_FOR_MODE[mode] || []).filter(t => results[t]);
  const allTabs = [...baseTabs, ...extraTabs.filter(t => results[t.key])];
  const [active, setActive] = useState(null);
  const firstKey = allTabs.length > 0 ? (typeof allTabs[0] === "string" ? allTabs[0] : allTabs[0].key) : null;
  const currentActive = active || firstKey;
  ...
  return (
    <div>
      <div className="tabs">
        {allTabs.map((t) => (
          <button key={getKey(t)} className={`tab-btn${currentActive === getKey(t) ? " active" : ""}`}
            onClick={() => setActive(getKey(t))}>
      ...
      <div className="result-box">
        {getContent(currentActive)
          ? <div className="result-text">{getContent(currentActive)}</div>
        ...
        {selectedFormats.map(fk => {
          ...
          return (
            <button key={fk} className={`choice-btn${activeFormat === fk ? " selected" : ""}`}
              style={{ fontSize: 12, padding: "4px 10px" }} onClick={() => setActiveFormat(fk)}>
              {f?.label}
            </button>
          );
        })}
...
export default function Home() {
  ...
  const resetAll = () => {
    ...
    setNotes(""); setResearchBrand(false);
    setAgentStatus({}); setResults({}); setDone(false); setError("");
    setClarificationQs(""); setClarificationAnswers(""); setResearchContent("");
    setWantsAnalyst(null); setWantsSocial(null); setSocialFromStrategyType(null);
    setSocialFromStrategyTonality(null); setSocialDone(false);
    ...
    setDesignTextChoice(null); setDesignText("");
    setSelectedFormats(DESIGN_FORMATS.map(f => f.key));
    setDesignVariants([]); setDesignGenerating(false); setDesignError("");
    setFeedbackText(""); setDesignIteration(0);
  };

  const getFiles = () => [
    ...briefFiles, ...socialProductFiles,
  ...
  const streamFromAPI = async (payload, path = "/api/pipeline") => {
    ...
    if (!res.ok) {
      ...
      throw new Error(e.error || `Blad serwera (${res.status})`);
    }
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      setError(e.message || "Blad polaczenia. Sprawdz konsole przegladarki (F12).");
    }
  };
  ...
  const runAfterClarification = async () => {
    ...
    try {
      ...
      console.error("AfterClarification error:", e);
      setError(e.message || "Blad polaczenia. Sprawdz konsole przegladarki (F12).");
    }
  };

  const runAnalystNow = async () => {
    setWantsAnalyst(true); setError("");
  ...
  const runSocialFromStrategy = async () => {
    ...
    try {
      ...
      setError(e.message); setSocialDone(true);
    }
  };
  ...
  const runDesign = async (iteration = 0, feedback = "") => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Blad API (${res.status}): ${e.slice(0, 100)}`);
      }
      const reader = res.body.getReader();
      const dec = new TextDecoder();
      let buf = "";
  ...
  const downloadWord = async () => {
    ...
    if (!res.ok) { alert("Blad eksportu Word. Sprobuj ponownie."); return; }
  ...
  const isRunning = ["running", "running_analyst", "running_social"].includes(step);
  const activeSteps = mode ? (STEPS_FOR_MODE[mode] || []) : [];
  const currentSteps = step === "running_analyst" ? ["analyst"]
    : step === "running_social" ? ["social_content"]
    : activeSteps;
  ...
  const showDesignFlow = mode === "design" ||
    (done && (mode === "social" || mode === "concept" || mode === "strategy"));

  const designStarted = step === "design_assets" || isPast("design_assets");
  const designReady = mode === "design" ? designStarted : (wantsDesign === true);

  const BackBtn = () => (
    <button className="btn" style={{ marginRight: "auto" }} onClick={goBack}>← Wróć</button>
  );

  const extraResultTabs = [
    ...(results["social_content"] ? [{ key: "social_content", label: "📱 Posty/Rolki" }] : []),
    ...(results["analyst"] && !STEPS_FOR_MODE[mode]?.includes("analyst")
      ? [{ key: "analyst", label: "📊 Plan mediowy" }] : []),
  ];
    ...
    <div className="app">
      <div className="header">
        ...
        <div>
          ...
          <div className="header-sub">Twoj partner od kreacji i strategii</div>
        </div>
      </div>
      ...
      <div className="messages">
            ...
            <div className="choices">
              {Object.entries(MODES).map(([key, m]) => (
                <button key={key} className="choice-btn" onClick={() => {
                  setMode(key);
                  if (key === "social") goNext("social_type");
                  else if (key === "design") goNext("design_brief");
                  else goNext("brief");
        ...
        {/* DESIGN MODE - direct brief */}
        {mode === "design" && (isPast("design_brief") || step === "design_brief") && (
          <>
            <RomanMsg>
              Opisz post lub projekt do ktorego chcesz grafike. Mozesz tez wgrac brief.
              {step === "design_brief" && (
                <>
                  <DropZone files={briefFiles} onFiles={setBriefFiles} label="Wgraj brief lub opis (PDF, TXT...)" />
                  <div className="input-area" style={{ marginTop: 10 }}>
                    <textarea className="msg-input" value={briefText}
                      onChange={e => setBriefText(e.target.value)}
                      placeholder="Opisz post lub projekt: np. Post o letniej wyprzedazy, wesoly ton, centrum handlowe M1 Marki..." />
                  </div>
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary"
                      disabled={!briefText.trim() && briefFiles.length === 0}
                      onClick={() => goNext("design_assets")}>Dalej</button>
                  </div>
                </>
              )}
            </RomanMsg>
            {isPast("design_brief") && (
              <UserMsg>{briefText ? briefText.slice(0,60)+"..." : briefFiles.map(f=>f.name).join(", ")}</UserMsg>
            )}
          </>
        )}
                ...
                <>
                  <div className="choices">
                    {Object.entries(SOCIAL_TYPES).map(([k, v]) => (
                      <button key={k} className="choice-btn"
                        onClick={() => { setSocialType(k); goNext("social_focus"); }}>{v}</button>
                    ))}
                  </div>
        ...
        {mode === "social" && (isPast("social_focus") || step === "social_focus") && (
          <>
                ...
                <>
                  <div className="choices">
                    <button className="choice-btn"
                      onClick={() => { setSocialFocus("specific"); goNext("social_product"); }}>
                      Konkretny produkt/wydarzenie
                    </button>
                    <button className="choice-btn"
                      onClick={() => { setSocialFocus("general"); goNext("brief"); }}>
                      Ogolnie marka
                    </button>
                  </div>
                  <div className="btn-row"><BackBtn /></div>
            ...
            {isPast("social_focus") && socialFocus && (
              <UserMsg>{socialFocus === "specific" ? "Konkretny produkt/wydarzenie" : "Ogolnie marka"}</UserMsg>
            )}
          </>
        )}
        ...
        {mode === "social" && socialFocus === "specific" && (isPast("social_product") || step === "social_product") && (
          <>
            <RomanMsg>
              ...
              {step === "social_product" && (
                <>
                  <DropZone files={socialProductFiles} onFiles={setSocialProductFiles} label="Wgraj materialy o produkcie" />
                  <div className="input-area" style={{ marginTop: 10 }}>
                    <textarea className="msg-input" value={socialProduct}
                      onChange={e => setSocialProduct(e.target.value)}
                      placeholder="Opisz produkt lub wydarzenie..." />
                  </div>
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary"
            ...
            {isPast("social_product") && (
              <UserMsg>{socialProduct ? socialProduct.slice(0,60)+"..." : socialProductFiles.map(f=>f.name).join(", ")}</UserMsg>
            )}
          </>
        )}

        {/* BRIEF */}
        {mode !== "design" && (isPast("brief") || step === "brief") && (
          <>
            <RomanMsg>
              {mode === "ads" ? "Wgraj brief i dane klienta." : "Wgraj brief lub opisz projekt tekstem."}
              {step === "brief" && (
                <>
                  <DropZone files={briefFiles} onFiles={setBriefFiles} label="Wgraj brief (PDF, Word, Excel, TXT...)" />
                  <div className="input-area" style={{ marginTop: 10 }}>
                    <textarea className="msg-input" value={briefText}
                      onChange={e => setBriefText(e.target.value)}
                      placeholder="Marka, produkt/usluga, cel kampanii, grupa docelowa, budzet, rynek..." />
                  </div>
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary"
                      disabled={!briefText.trim() && briefFiles.length === 0}
                      onClick={() => goNext(mode === "social" ? "social_tonality" : "products")}>
                      Dalej
                    </button>
                  </div>
                </>
                ...
                <>
                  <div className="choices">
                    {Object.entries(TONALITY_TYPES).map(([k, v]) => (
                      <button key={k} className={`choice-btn${socialTonality === k ? " selected" : ""}`}
                        onClick={() => setSocialTonality(k)}>{v}</button>
                    ))}
                  </div>
                  <div className="btn-row">
                    <BackBtn />
        ...
        {mode === "social" && (isPast("examples") || step === "examples") && (
          <>
                ...
                <>
                  ...
                  <div className="btn-row">
                    ...
                    <button className="btn btn-primary" onClick={() => goNext("products")}>
                      {exampleFiles.length > 0 ? "Dalej" : "Pominaj"}
                    </button>
                  </div>
                </>
              )}
            ...
            {isPast("examples") && (
              <UserMsg>{exampleFiles.length > 0 ? exampleFiles.map(f=>f.name).join(", ") : "Pominieto"}</UserMsg>
            )}
          </>
        )}

        {/* PRODUCTS */}
        {mode !== "design" && (isPast("products") || step === "products") && (
          <>
            <RomanMsg>
              ...
              <div style={{ fontSize: 12, color: "var(--text-hint)", marginTop: 4 }}>
                Jesli nie - Roman przeprowadzi research samodzielnie
              </div>
              {step === "products" && (
                <>
                  ...
                  <div className="btn-row">
                    ...
                    <button className="btn"
                      onClick={() => { setProductFiles([]); setResearchBrand(true); goNext("notes"); }}>
                      Szukaj samodzielnie
                    </button>
                    <button className="btn btn-primary"
                      onClick={() => { setResearchBrand(false); goNext("notes"); }}>
                      {productFiles.length > 0 ? "Dalej" : "Pominaj"}
                    </button>
                  </div>
                </>
              )}
            ...
            {isPast("products") && (
              <UserMsg>
                {researchBrand ? "Szukaj samodzielnie"
                  : productFiles.length > 0 ? productFiles.map(f=>f.name).join(", ")
                  : "Brak materialow"}
              </UserMsg>
            )}
          </>
        ...
        {/* NOTES */}
        {mode !== "design" && (isPast("notes") || step === "notes") && (
          <>
            <RomanMsg>
              ...
              {step === "notes" && (
                <>
                  ...
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary" onClick={runPipeline}>Dzialaj Romek 🚀</button>
                  </div>
                </>
              )}
            </RomanMsg>
                ...
                <>
                  ...
                  <div className="btn-row">
                    <button className="btn btn-primary" disabled={!clarificationAnswers.trim()}
                      onClick={runAfterClarification}>Dzialaj Romek 🚀</button>
                  </div>
                </>
              )}
            </RomanMsg>
            {isPast("clarification") && clarificationAnswers && (
              <UserMsg>{clarificationAnswers.slice(0,60)}</UserMsg>
            )}
          </>
        )}
          ...
          <RomanMsg>
            {!error && (
              <div style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 8 }}>
                ...
                Pracuje... to moze zajac 30-90 sekund. Nie zamykaj strony.
              </div>
            )}
                ...
                {currentSteps.map(id => (
                  <div key={id} className={stepClass(id)}>
                    <span className="step-icon">
                      {agentStatus[id] === "running" ? <span className="spin">⟳</span> : AGENT_LABELS[id]?.icon}
                    </span>
            ...
            {error && (
              <div className="error-box">
                ...
                <div style={{ marginTop: 10 }}>
                  <button className="btn" onClick={goBack}>Wróc i sprobuj ponownie</button>
                </div>
              </div>
            )}
            {Object.keys(results).length > 0 && (
              <ResultTabs mode={mode} results={results} extraTabs={extraResultTabs} />
            )}
          </RomanMsg>
        )}
        ...
        {/* POST-PIPELINE: Czy plan reklamowy? */}
        {done && mode !== "ads" && mode !== "design" && wantsAnalyst === null && (
          <RomanMsg>
        ...
        {/* POST-PIPELINE: Czy posty/rolki? */}
        {done && (mode === "concept" || mode === "strategy") && wantsAnalyst !== null && wantsSocial === null && (
          <RomanMsg>
            Czy przygotowac posty lub rolki na podstawie tego konceptu/strategii?
            <div className="choices">
        ...
        {done && wantsSocial === true && socialFromStrategyType !== null && socialFromStrategyTonality === null && (
          <RomanMsg>
            ...
            <div className="choices">
              {Object.entries(TONALITY_TYPES).map(([k, v]) => (
                <button key={k} className="choice-btn"
                  onClick={() => { setSocialFromStrategyTonality(k); runSocialFromStrategy(); }}>{v}</button>
              ))}
            </div>
          </RomanMsg>
        )}

        {/* POST-PIPELINE: Czy grafiki? */}
        {showDesignFlow && mode !== "design" && wantsDesign === null
          && (mode === "social" || (wantsAnalyst !== null && (wantsSocial !== null || (mode !== "concept" && mode !== "strategy"))))
          && (wantsSocial !== true || socialDone) && done && (
          <RomanMsg>
        ...
        {/* POST SELECT */}
        {designReady && (mode === "social" || (wantsSocial && results["social_content"]))
          && (isPast("design_post_select") || step === "design_post_select") && (
          <>
            <RomanMsg>
              Do ktorego posta chcesz grafike? Wklej jego tresc.
              {step === "design_post_select" && (
                <>
                  <div className="input-area" style={{ marginTop: 10 }}>
                    <textarea className="msg-input" value={selectedPostForDesign}
                      onChange={e => setSelectedPostForDesign(e.target.value)}
                      placeholder="Wklej tresc wybranego posta..." style={{ minHeight: 80 }} />
                  </div>
                  <div className="btn-row">
                    <button className="btn" onClick={() => goNext("design_assets")}>Pominaj (ogolna grafika)</button>
                    <button className="btn btn-primary" disabled={!selectedPostForDesign.trim()}
                      onClick={() => goNext("design_assets")}>Dalej</button>
                  </div>
                </>
              )}
            </RomanMsg>
            {isPast("design_post_select") && (
              <UserMsg>{selectedPostForDesign ? selectedPostForDesign.slice(0,60)+"..." : "Ogolna grafika"}</UserMsg>
            )}
          </>
        )}

        {/* ASSETS */}
        {designReady && (isPast("design_assets") || step === "design_assets") && (
          <>
            <RomanMsg>
              ...
              {step === "design_assets" && (
                <>
                  <div className="choices">
                    <button className="choice-btn"
                      onClick={() => { setHasBrandAssets(true); goNext("design_upload"); }}>Tak, wgram</button>
                    <button className="choice-btn"
                      onClick={() => { setHasBrandAssets(false); goNext("design_position"); }}>Nie - Roman robi co moze</button>
                  </div>
                  {mode !== "design" && <div className="btn-row"><BackBtn /></div>}
                </>
              )}
            </RomanMsg>
        ...
        {/* UPLOAD ASSETS */}
        {designReady && hasBrandAssets && (isPast("design_upload") || step === "design_upload") && (
          <>
            <RomanMsg>
              ...
              {step === "design_upload" && (
                <>
                  ...
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary" onClick={() => goNext("design_position")}>Dalej</button>
                  </div>
                </>
              )}
            </RomanMsg>
            {isPast("design_upload") && (
              <UserMsg>Logo: {logoFile[0]?.name || "brak"} | Font: {fontFile[0]?.name || "brak"}</UserMsg>
            )}
          </>
        )}

        {/* LOGO POSITION */}
        {designReady && (isPast("design_position") || step === "design_position") && (
          <>
            <RomanMsg>
              Gdzie ma byc logo?
                ...
                <>
                  <div className="choices">
                    {Object.entries(LOGO_POSITIONS).map(([k, v]) => (
                      <button key={k} className={`choice-btn${logoPosition === k ? " selected" : ""}`}
                        onClick={() => setLogoPosition(k)}>{v}</button>
                    ))}
                  </div>
                  <div className="btn-row">
                    <BackBtn />
                    <button className="btn btn-primary" disabled={!logoPosition}
                      onClick={() => goNext("design_text")}>Dalej</button>
                  </div>
                </>
              )}
            </RomanMsg>
        ...
        {/* TEXT */}
        {designReady && (isPast("design_text") || step === "design_text") && (
          <>
            <RomanMsg>
              ...
              {step === "design_text" && (
                <>
                  ...
                  {designTextChoice === "custom" && (
                    <div className="input-area" style={{ marginTop: 10 }}>
                      <textarea className="msg-input" value={designText}
            ...
            {isPast("design_text") && designTextChoice && (
              <UserMsg>{designTextChoice === "custom" ? `"${designText}"` : "Bez tekstu"}</UserMsg>
            )}
          </>
        )}

        {/* FORMATS */}
        {designReady && (isPast("design_formats") || step === "design_formats") && (
          <>
            <RomanMsg>
              Ktore formaty?
              {step === "design_formats" && (
                <>
                  <div className="choices">
                    {DESIGN_FORMATS.map(f => (
                      <button key={f.key}
                        className={`choice-btn${selectedFormats.includes(f.key) ? " selected" : ""}`}
                        onClick={() => setSelectedFormats(p =>
            ...
            {isPast("design_formats") && (
              <UserMsg>{selectedFormats.map(k => DESIGN_FORMATS.find(f=>f.key===k)?.label).join(", ")}</UserMsg>
            )}
          </>
        )}

        {/* DESIGN RESULTS */}
        {designReady && (step === "design_generating" || designVariants.length > 0 || designError) && (
          <RomanMsg>
            ...
            {!designGenerating && designVariants.length === 0 && !designError && step === "design_generating" && (
              <div className="error-box">
                Nie udalo sie wygenerowac wizualizacji. Sprawdz GOOGLE_API_KEY w Vercel.
            ...
            {designVariants.length >= 2 && !designGenerating && (
              <>
                ...
                <div className="input-area">
                  <textarea className="msg-input" value={feedbackText}
                    onChange={e => setFeedbackText(e.target.value)}
                    placeholder="np. Za ciemne tlo, logo za male, bardziej minimalistycznie..."
                    style={{ minHeight: 70 }} />
                </div>
                <div className="btn-row">
                  <button className="btn" onClick={resetAll}>Nowy brief</button>
